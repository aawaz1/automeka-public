...
const Container = ({ isOpen, handleIsOpen }) => {
    ...
    return (
        <>
            {
                isOpen && (<div style={{ zIndex: 100, right: isOpen ? 0 : '-144vw' }} className={`fixed top-0 overflow-y-scroll w-[17rem] h-[100vh] bg-white`}><CartMenu handleIsOpen={handleIsOpen} /></div>)
            }
            {
