...
const Category = () => {
    ...
    const featuredProducts = useFeaturedProducts(search);
    ...
    const filteredProduct = (products) => {
        ...
        return products.filter(item => (!selectedCategory || item.category.name === selectedCategory))
                    ...
                    <Grid item lg={2} md={3} xs={12}>
                        <div className='hidden md:block   border border-2px border-solid rounded-sm  justify-center shadow-lg border-gray-400 w-[100%]'>
                            ...
                            <div className='p-4 flex flex-wrap gap-2'>
                                {categories.map(category => {
                                    return (
                                    <button className={`text-[0.9rem] font-medium font-poppins border border-black border-20px p-1 px-2 rounded-sm hover:bg-orange-400 hover:text-white ${selectedCategory === category?.name ? "bg-orange-400 text-white" : ""}`} onClick={() => setSelectedCategory(category.name)}>{category.name}</button>
                                    )
                            ...
                            <h2 className='text-[1rem] pl-4 pt-3 font-semibold '>filters</h2>
                            <div className='pb-1'>   <hr className='border border-solid border-gray-400 w-[80%] m-auto p-0' /></div>
                            <button className={`text-[0.9rem] font-medium font-poppins border border-black border-20px p-1 px-2 rounded-sm hover:bg-orange-400 hover:text-white ${selectedCategory === category?.name ? "bg-orange-400 text-white" : ""}`} onClick={() => setSelectedCategory(category.name)}>Top Selling</button>
